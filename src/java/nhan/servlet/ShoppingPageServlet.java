...
public class ShoppingPageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = siteMaps.getProperty(
                MyApplicationConstants.ShoppingPageFeature.ERROR_PAGE);
        try {
            // 1. Call dao
            ProductDAO dao = new ProductDAO();
            //2. call method
            dao.getItemFromInventory();
            //3. 
            List<ProductDTO> listItems = dao.getProductList();
            request.setAttribute("ITEMS_LIST", listItems);
            url = siteMaps.getProperty(
                MyApplicationConstants.ShoppingPageFeature.SHOPPING_PAGE);
        } catch (NamingException e) {
            log("Error at ShoppingPageServlet" + e.getMessage());
        } catch (SQLException ex) {
            log("Error at ShoppingPageServlet" + ex.getMessage());
        } finally {
